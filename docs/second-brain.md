...
같은 루프의 **두 번째 접점이 웹 UI다** (에이전트 경유 없이): `canopy serve`의
홈이 매일 재발견 카드 1개(+bridge 카드)를 띄우고, 👍/👎/😴 버튼이 CLI
`resurface feedback`과 **같은 state 파일**에 기록한다 — 쿨다운·피드백이
공유되므로 웹에서 반응한 페이지는 에이전트 브리핑에도 중복 노출되지 않는다.
설계는 [web-ui-plan-2.md](web-ui-plan-2.md) D6.
...
이 루프의 **수요 측 신호**도 수집된다: 웹 UI에서 결과가 빈약했던 검색은
`_meta/webui/search-gaps.jsonl`에 쌓이고 `점검 → 검색 갭` 화면에 집계된다.
"위키가 답하지 못한 질문"의 목록은 곧 파일링 후보 목록이다 — 에이전트가 이
파일을 읽고 페이지 초안을 제안할 수 있다.
...
- 커버리지 갭 후보: raw/ 최근 유입과 위키 페이지의 의미적 커버리지 비교
  (원 설계 전략 5 — 검색 갭 로그가 그 수요 측 절반을 이미 구현했다;
  남은 것은 공급 측: raw/ 임베딩과의 비교)
- 읽기 이력(`_meta/webui/reads.json`)을 resurface의 진짜 열람 신호로 사용 —
  지금은 updated 날짜로 "잊힘"을 근사하는데, reads가 쌓이면 실제 주의 데이터로
  대체할 수 있다 (보드 Icebox)
