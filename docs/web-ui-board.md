...
_(비어 있음 — **M1–M10 전 마일스톤 완료.** 남은 것은 Icebox의 조건부 항목뿐)_

## Done

### M10+ — 섬(단절 클러스터) 검출과 다리 제안

- [x] `wiki.Components()`: 무향 링크 그래프 연결 성분 분석 (최대 성분 = 본토, 나머지 = 섬)
- [x] `canopy lint`에 `island` 검출 추가(warning, 구성원 나열) — 고아 검사가 놓치는
      "서로는 연결된 고립 클러스터"를 잡는다. invariants B2 추가
- [x] 웹 점검 페이지: 섬별 구성원 + **다리 제안**(임베딩으로 섬↔본토 최고 유사도 페어,
      "이 링크 하나면 연결" + 바로 편집 링크). 자동 연결은 하지 않음(링크는 콘텐츠 판단)
- [x] 그래프 페이지: 섬 노드에 노란 점선 halo + 범례
- [x] hover 툴팁: 줌아웃으로 라벨이 사라져도 hover 노드는 항상 정보 카드
      (제목·분류·링크 수·읽음·섬 여부·발췌문). 발췌문은 /api/graph 페이로드에
      선적재해 비동기 없이 즉시 표시(+30KB, fetch/캐시/갱신 코드 제거)
- ✓ Exit:
  - [x] 실위키에서 섬 3개(요리·건강 9p, 수학 3p, 터미널 3p) 검출·표시·다리 제안 확인

### M10 — 인터랙티브 지식 그래프

- [x] `GET /api/graph`: 전체 페이지 노드 + 해석된 위키링크 엣지 JSON (dir·degree·읽음 여부 포함)
- [x] `GET /graph`: 전체 화면 캔버스 그래프 — force 레이아웃, 줌/팬/노드 드래그
      (vendor: force-graph 173KB, CDN 없음)
- [x] 색 = 디렉토리(범례), 크기 = degree, **안 읽은 페이지는 붉은 링** (읽기 히스토리 연동)
- [x] hover 시 이웃만 하이라이트 + 링크 파티클, 줌인하면 라벨 표시(Obsidian 동작),
      노드 클릭 → 페이지 이동
- [x] 노드 찾기 박스: Enter로 해당 노드로 비행(centerAt+zoom) + 이웃 하이라이트
- ✓ Exit:
  - [x] 149노드/564엣지 실위키에서 줌·라벨·하이라이트·검색 비행을 Chrome으로 확인
...
_(비어 있음 — 계획된 마일스톤 M1–M10 전부 완료)_

### Icebox (지금 할 일 아님 — 명시된 조건이 생기면 Backlog로 승격)

- **웹 편집의 LLM 위임 전환** — "직접 저장" 대신 웹에서 수정 요청을 남기면
  에이전트가 콘텐츠 규칙을 지키며 처리하는 경로 (philosophy 원칙 9의 전제;
  에이전트 상주/큐 구조가 정해지면)
