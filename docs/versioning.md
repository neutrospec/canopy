...
시작 시 판정 전체 — 아래 세 절(어떻게 도는가·기준선 판정·다운그레이드 가드)을
한 장으로:

```mermaid
flowchart TD
    ST["명령 시작 — migrate.Ensure()"] --> EX{"state.json 있음?"}
    EX -- "없음 (첫 실행)" --> TR{"디스크에 canopy 흔적?<br/>(설정·모델·캐시·레거시 ~/.canopy)"}
    TR -- "없음 — 완전 신규 설치" --> TG["Target에서 시작<br/>(올릴 옛 상태가 없음)"]
    TR -- "있음 — 버전 관리 이전 설치" --> Z["rung 0에서 시작<br/>(사다리 전체 재생 — idempotent라 안전)"]
    EX -- "있음" --> GT{"기록 rung > Target?"}
    GT -- "예 — 더 새 canopy가 쓰던 데이터" --> RF["거부·종료: canopy를 업그레이드하라 (J4)"]
    GT -- "아니오" --> CL
    Z --> CL["기록 rung+1부터 Target까지 차례로 실행,<br/>한 단계 끝날 때마다 각인"]
    CL --> DN["완료 — 실패 시 마지막 성공 rung에서 재개"]
```
...
**언제 필요한가** — 재현 불가 **머신-로컬** 상태를 옛 데이터가 만족하지 못하는
방식으로 바꿀 때. 예: 설정 필드를 이름 바꾸거나 비자명한 기본값과 함께 추가,
디렉토리 이동, `$XDG_STATE_HOME`의 이벤트 DB 스키마 파괴적 변경.

**사다리 대상이 아닌 것 ①** — 위키와 함께 여행하는 파일(`_meta/*`). 사다리 기록은
머신-로컬이라 옛 형식의 위키를 clone하면 다시 돌지 않는다. 이 파일들은 **파일 안의
`version` 필드로 self-version**한다(관용적 Load, additive 변경, 구버전의 재저장이
새 필드를 깨뜨릴 위험이 있으면 별도 파일 분리 — AGENTS.md 규칙 1b).
